...
public class SvcWeeklyReport extends BaseReport {
	...
	public SvcWeeklyReport(UserInfo userInfo) {
		...
		for (Entry<String, Object> e : hData.entrySet()) {
			...
			switch (e.getKey()) {
				...
				case "opt01" : case "opt02" : case "opt03" : case "opt04" : 
					if (nC1++ == 0) {
						contentsList.add(++ItemNo + ". " + StringUtil.convertString(hMap.get("cscoNm")) + " 주간 보안관제 업무현황");
						reportData.put("C1", ItemNo + ". " + StringUtil.convertString(hMap.get("cscoNm")) + " 주간 보안관제 업무현황");
					}
					break;
				//침해위협 탐지현황
	...
	private void ServiceDeskStatItem06(HashMap<String, Object> reportData, int cscoCd, String sStartDay, String sEndDay, boolean bChk) throws Exception {
		...
		List<HashMap<String, Object>> dbBefore = svcDao.selectViolAlrtTopN(cscoCd, baseDao.addDate(sStartDay, -7), baseDao.addDate(sEndDay, -7), 0, "");
	...
	private void ServiceDeskStatItem13(HashMap<String, Object> reportData, String sStartDay, String sEndDay) throws Exception {
		...
		List<HashMap<String, Object>> dbBefore = svcDao.selectTotalTrendTopN(baseDao.addDate(sStartDay, -7), baseDao.addDate(sEndDay, -7), 1, 0);
