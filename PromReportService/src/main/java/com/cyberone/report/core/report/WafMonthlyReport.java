...
public class WafMonthlyReport extends BaseReport {
	...
	public HashMap<String, Object> getDataSource(String sReportType, String sSearchDate, HashMap<String, Object> hMap, int ItemNo, List<String> contentsList) throws Exception {
		...
		contentsList.add(ItemNo + ". " + (String)hMap.get("assetName") + " 장비의 월간 웹방화벽 탐지로그 분석");
		reportData.put("RT", ItemNo + ". " + (String)hMap.get("assetName") + " 장비의 월간 웹방화벽 탐지로그 분석"); //Report Title
		...
		for (Entry<String, Object> e : hData.entrySet()) {
			...
			switch (e.getKey()) {
				...
				case "opt01" : case "opt02" :
					if (nC1 == 0) {
						reportData.put("C1", push(contentsList, "  ", ItemNo, ++nC1, " 월간 탐지 추이"));
					}
					break;
				//이벤트 현황
				case "opt03" :
					if (nC2 == 0) {
						reportData.put("C2", push(contentsList, "  ", ItemNo, (++nC2 + nC1), " 월간 탐지 현황"));
					}
					break;
				//출발지IP 현황
				case "opt04" :
					if (nC3 == 0) {
						reportData.put("C3", push(contentsList, "  ", ItemNo, (++nC3 + nC2 + nC1), " 월간 출발지 현황"));
					}
					break;
				//도메인별 상세통계
				case "opt05" : case "opt06" : case "opt07" :
					if (nC4 == 0) {
						reportData.put("C4", push(contentsList, "  ", ItemNo, (++nC4 + nC3 + nC2 + nC1), " 월간 목적지 현황"));
					}
					break;
				//성능정보
				case "opt99" :
	...
	private void All_DetectLog_Trend(HashMap<String, Object> reportData, int assetCode, String sStartDay, String sEndDay) throws Exception {
		...
		Iterable<DBObject> dbResult = wafDao.DetectLogTrend("MON", "", assetCode, wafDao.addMonth(sStartDay,-5), sEndDay);
		...
		dbResult = wafDao.DetectLogTrend("MON", ALLOW, assetCode, wafDao.addMonth(sStartDay, -5), sEndDay);
	...
	private void Domain_TopN_Trend(HashMap<String, Object> reportData, int assetCode, String sStartDay, String sEndDay, int nChoice) throws Exception {
    	...
    	if (nChoice == 1) { //도메인 TOP10 일별 발생추이
			...
			for (DBObject mVal : dbTopN) {
	    		...
	    		Iterable<DBObject> dbResult = wafDao.DomainTopNTrend("MON", "", assetCode, wafDao.addMonth(sStartDay,-5), sEndDay, sHost);
	    		...
	    		dbResult = wafDao.DomainTopNTrend("MON", ALLOW, assetCode, wafDao.addMonth(sStartDay,-5), sEndDay, sHost);
