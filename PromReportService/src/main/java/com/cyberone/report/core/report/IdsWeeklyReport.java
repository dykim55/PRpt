...
public class IdsWeeklyReport extends BaseReport {
	...
	public IdsWeeklyReport(UserInfo userInfo) {
	...
	public HashMap<String, Object> getDataSource(String sReportType, String sSearchDate, HashMap<String, Object> hMap, int ItemNo, List<String> contentsList) throws Exception {
		...
		contentsList.add(ItemNo + ". " + (String)hMap.get("assetName") + " 장비의 주간 IDS 탐지로그 분석");
		reportData.put("RT", ItemNo + ". " + (String)hMap.get("assetName") + " 장비의 주간 IDS 탐지로그 분석");
		...
		for (Entry<String, Object> e : hData.entrySet()) {
			...
			switch (e.getKey()) {
				...
				case "opt01" : case "opt02" : case "opt03" :  
					if (nC1 == 0) { 
						contentsList.add("  " + ItemNo + "." + ++nC1 + " 주간 탐지 추이");
						reportData.put("C1", "  " + ItemNo + "." + nC1 + " 주간 탐지 추이");
					}
					break;
				//이벤트 현황
				case "opt04" : case "opt05" : case "opt06" : case "opt07" : case "opt08" :  
					if (nC2 == 0) { 
						contentsList.add("  " + ItemNo + "." + (++nC2 + nC1) + " 주간 탐지 현황");
						reportData.put("C2", "  " + ItemNo + "." + (nC2 + nC1) + " 주간 탐지 현황");
					}
					break;
				//출발지IP 현황
				case "opt09" : case "opt10" :  
					if (nC3 == 0) { 
						contentsList.add("  " + ItemNo + "." + (++nC3 + nC2 + nC1) + " 주간 출발지 현황");
						reportData.put("C3", "  " + ItemNo + "." + (nC3 + nC2 + nC1) + " 주간 출발지 현황");
					}
					break;
				//목적지IP 현황
				case "opt11" : case "opt12" :  
					if (nC4 == 0) { 
						contentsList.add("  " + ItemNo + "." + (++nC4 + nC3 + nC2 + nC1) + " 주간 목적지 현황");
						reportData.put("C4", "  " + ItemNo + "." + (nC4 + nC3 + nC2 + nC1) + " 주간 목적지 현황");
					}
					break;
				//서비스 현황
	...
	private void Direction_EventTopN(HashMap<String, Object> reportData, int nDirection, int assetCode, String sStartDay, String sEndDay, int nLimit, boolean bChk) throws Exception {
    	...
    	Iterable<DBObject> dbBefore = idsDao.EventTopN("DY", nDirection, assetCode, idsDao.addDate(sStartDay,-7), idsDao.addDate(sEndDay,-7), 0);
	...
	private void New_DetectLog_Condition(HashMap<String, Object> reportData, int nDirection, int assetCode, String sStartDay, String sEndDay) throws Exception {
		...
		//전주 탐지 이벤트리스트
		Iterable<DBObject> dbResult = idsDao.EventTopN("DY", nDirection, assetCode, idsDao.addDate(sStartDay,-7), idsDao.addDate(sEndDay,-7), null);
    	for (DBObject val : dbResult) {
	...
	private void Double_DetectLog_Condition(HashMap<String, Object> reportData, int nDirection, int assetCode, String sStartDay, String sEndDay) throws Exception {
		...
		//전일 탐지 이벤트리스트
		Iterable<DBObject> dbBefore = idsDao.EventTopN("DY", nDirection, assetCode, idsDao.addDate(sStartDay,-7), idsDao.addDate(sEndDay,-7), null);
		HashMap<String, DBObject> mapBefore = new HashMap<String, DBObject>();
    	for (DBObject val : dbBefore) {
    		mapBefore.put((String)val.get("message"), val);
    	}
	...
	private void ALL_DetectLog_TopN(HashMap<String, Object> reportData, int nDirection, int assetCode, String sStartDay, String sEndDay, boolean bSrcIp, int nLimit, boolean bChk) throws Exception {
    	...
    	//전일 현황
    	Iterable<DBObject> dbBefore = idsDao.BeforeIpCondition("DY", nDirection, assetCode, idsDao.addDate(sStartDay,-7), idsDao.addDate(sEndDay,-7), bSrcIp, topNIps);
    	HashMap<String, DBObject> mapBefore = new HashMap<String, DBObject>();
    	for (DBObject val : dbBefore) {
    		if (bSrcIp) {
	...
	private void ALL_ServiceTop10(HashMap<String, Object> reportData, int nDirection, int assetCode, String sStartDay, String sEndDay, boolean bChk) throws Exception {
    	...
    	if (bChk) { //서비스 TOP10 증감현황 및 이벤트유형
    		...
    		dbResult = idsDao.ServiceDetectLog("DY", nDirection, assetCode, idsDao.addDate(sStartDay,-7), idsDao.addDate(sEndDay,-7), 0);
        	HashMap<String, DBObject> mapBefore = new HashMap<String, DBObject>(); 
        	for (DBObject val : dbResult) {
