...
public class IdsMonthlyReport extends BaseReport {
	...
	public HashMap<String, Object> getDataSource(String sReportType, String sSearchDate, HashMap<String, Object> hMap, int ItemNo, List<String> contentsList) throws Exception {
		...
		contentsList.add(ItemNo + ". " + (String)hMap.get("assetName") + " 장비의 월간 IDS 탐지로그 분석");
		reportData.put("RT", ItemNo + ". " + (String)hMap.get("assetName") + " 장비의 월간 IDS 탐지로그 분석");
		...
		for (Entry<String, Object> e : hData.entrySet()) {
			...
			switch (e.getKey()) {
				...
				case "opt01" : case "opt02" : case "opt03" :  
					if (nC1 == 0) { 
						contentsList.add("  " + ItemNo + "." + ++nC1 + " 월간 탐지 추이");
						reportData.put("C1", "  " + ItemNo + "." + nC1 + " 월간 탐지 추이");
					}
					break;
				//이벤트 현황
				case "opt04" : case "opt05" : case "opt06" : case "opt07" : case "opt08" :  
					if (nC2 == 0) { 
						contentsList.add("  " + ItemNo + "." + (++nC2 + nC1) + " 월간 탐지 현황");
						reportData.put("C2", "  " + ItemNo + "." + (nC2 + nC1) + " 월간 탐지 현황");
					}
					break;
				//출발지IP 현황
				case "opt09" : case "opt10" :  
					if (nC3 == 0) { 
						contentsList.add("  " + ItemNo + "." + (++nC3 + nC2 + nC1) + " 월간 출발지 현황");
						reportData.put("C3", "  " + ItemNo + "." + (nC3 + nC2 + nC1) + " 월간 출발지 현황");
					}
					break;
				//목적지IP 현황
				case "opt11" : case "opt12" :  
					if (nC4 == 0) { 
						contentsList.add("  " + ItemNo + "." + (++nC4 + nC3 + nC2 + nC1) + " 월간 목적지 현황");
						reportData.put("C4", "  " + ItemNo + "." + (nC4 + nC3 + nC2 + nC1) + " 월간 목적지 현황");
					}
					break;
				//서비스 현황
	...
	private void ALL_ServiceTop10(HashMap<String, Object> reportData, int nDirection, int assetCode, String sStartDay, String sEndDay, boolean bChk) throws Exception {
    	...
    	if (bChk) { //서비스 TOP10 증감현황 및 이벤트유형
        	...
        	for (HashMap<String, Object> hMap : dataSource1) {
        		...
        		Iterable<DBObject> dbTmp = idsDao.ServiceEventTopN("MON", nDirection, assetCode, sStartDay, sEndDay, nPort);
