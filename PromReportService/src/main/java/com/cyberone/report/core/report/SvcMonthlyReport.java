...
public class SvcMonthlyReport extends BaseReport {
		...
		for (Entry<String, Object> e : hData.entrySet()) {
			...
			switch (e.getKey()) {
				...
				case "opt01" : case "opt02" : case "opt03" : case "opt04" : 
					if (nC1++ == 0) {
						contentsList.add(++ItemNo + ". " + StringUtil.convertString(hMap.get("cscoNm")) + " 월간 보안관제 업무현황");
						reportData.put("C1", ItemNo + ". " + StringUtil.convertString(hMap.get("cscoNm")) + " 월간 보안관제 업무현황");
					}
					break;
				//침해위협 탐지현황
				...
				case "opt13" : case "opt14" : case "opt15" :
					if (nC6++ == 0) { 
						contentsList.add(++ItemNo + ". 주간 싸이버원 침해위협탐지 동향 요약");
						reportData.put("C6", ItemNo + ". 주간 싸이버원 침해위협탐지 동향 요약");
					}
					break;
			}
			...
			switch (e.getKey()) {
				...
				case "opt05" : 
					...
					ServiceDeskStatItem05(reportData, cscoCd, sStartDay, sEndDay, nChoice);
					break;
				case "opt06" : 
				...
				case "opt07" : 
					...
					ServiceDeskStatItem07(reportData, cscoCd, sStartDay, sEndDay, nChoice, !StringUtil.isEmpty(hData.get("ck7")), StringUtil.convertString(hData.get("sd7")));
					break;
				case "opt08" : 
					reportData.put("opt08", push(contentsList, "  ", ItemNo, nS2++, " 출발지 국가별 현황"));
					ServiceDeskStatItem08(reportData, cscoCd, sStartDay, sEndDay, !StringUtil.isEmpty(hData.get("ck8")), StringUtil.convertString(hData.get("sd8")));
					break;
				case "opt09" : 
				...
				case "opt12" : 
					...
					break;
				case "opt13" :
					reportData.put("opt13", push(contentsList, "  ", ItemNo, nS3++, " 침해위협 유형별 탐지동향"));
					ServiceDeskStatItem13(reportData, sStartDay, sEndDay);
					break;
				case "opt14" :
					reportData.put("opt14", push(contentsList, "  ", ItemNo, nS3++, " 출발지 동향"));
					ServiceDeskStatItem14(reportData, sStartDay, sEndDay);
					break;
				case "opt15" :
					reportData.put("opt15", push(contentsList, "  ", ItemNo, nS3++, " 목적지 동향"));
					ServiceDeskStatItem15(reportData, sStartDay, sEndDay);
					break;
			}
		}
	...
	private void ServiceDeskStatItem05(HashMap<String, Object> reportData, int cscoCd, String sStartDay, String sEndDay, int nChoice) throws Exception {
	...
	private void ServiceDeskStatItem06(HashMap<String, Object> reportData, int cscoCd, String sStartDay, String sEndDay, boolean bChk) throws Exception {
		...
		List<HashMap<String, Object>> dbBefore = svcDao.selectViolAlrtTopN(cscoCd, baseDao.addMonth(sStartDay, -1), baseDao.addMonth(sEndDay, -1), 0, "");
	...
	private void ServiceDeskStatItem07(HashMap<String, Object> reportData, int cscoCd, String sStartDay, String sEndDay, int nChoice, boolean bChk, String sOpt) throws Exception {
    	...
    	if (bChk) {
	    	...
	    	if (Integer.valueOf(sOpt) == 1) { //최근 1개월
	        	...
	        	reportData.put("OPT7_3", "월");
	    	}
	    	reportData.put("OPT7_2", new SynthesisDataSource(dataSource2));
    	}
	}

	private void ServiceDeskStatItem08(HashMap<String, Object> reportData, int cscoCd, String sStartDay, String sEndDay, boolean bChk, String sOpt) throws Exception {
	...
	private void ServiceDeskStatItem13(HashMap<String, Object> reportData, String sStartDay, String sEndDay) throws Exception {
		...
		List<HashMap<String, Object>> dbBefore = svcDao.selectTotalTrendTopN(baseDao.addMonth(sStartDay, -1), baseDao.addMonth(sEndDay, -1), 1, 0);
		...
		for (HashMap<String, Object> hMap : resultList) {
			...
			HashMap<String, Object> prevMap = mapBefore.get((String)hMap.get("esmRuleGroupPrntCd"));
        	...
        	for (HashMap<String, Object> pMap : dbRuleGroup) {
        		...
        		if (prevMap != null) {
        			int nRank = (Integer)prevMap.get("rank") - nNo;
        			map.put("rank", nRank > 0 ? "↑ " + nRank : nRank < 0 ? "↓ " + Math.abs(nRank) : "-");
        		} else {
        			map.put("rank", "신규");
        		}
	...
	private void ServiceDeskStatItem14(HashMap<String, Object> reportData, String sStartDay, String sEndDay) throws Exception {
		...
		for (HashMap<String, Object> hMap : resultList) {
        	HashMap<String, Object> map = new HashMap<String, Object>();
        	map.put("category", (String)hMap.get("srcGeo"));
        	map.put("count", ((Long)hMap.get("CNT")).longValue());
    		lTotalCount += ((Long)hMap.get("CNT")).longValue();
    		dataSource1.add(map);
		}
		...
		for (HashMap<String, Object> hMap : resultList) {
    		...
    		for (HashMap<String, Object> mMap : prntList) {
            	...
            	for (HashMap<String, Object> pMap : dbRuleGroup) {
                	...
                	map.put("count", ((Long)mMap.get("CNT")).longValue());
            		map.put("message", (String)pMap.get("esmRuleGroupNm"));
	...
	private void ServiceDeskStatItem15(HashMap<String, Object> reportData, String sStartDay, String sEndDay) throws Exception {
		...
		for (HashMap<String, Object> hMap : resultList) {
        	HashMap<String, Object> map = new HashMap<String, Object>();
        	map.put("category", baseDao.getPortServiceName((Integer)hMap.get("destPort")));
        	map.put("count", ((Long)hMap.get("CNT")).longValue());
    		lTotalCount += ((Long)hMap.get("CNT")).longValue();
    		dataSource1.add(map);
		}
		...
		for (HashMap<String, Object> hMap : resultList) {
    		...
    		for (HashMap<String, Object> mMap : prntList) {
            	...
            	for (HashMap<String, Object> pMap : dbRuleGroup) {
                	HashMap<String, Object> map = new HashMap<String, Object>();
                	map.put("no", nNo);
                	map.put("destIp", baseDao.getPortServiceName(nPort));
                	map.put("category", (String)mMap.get("esmRuleGroupPrntNm"));
                	map.put("count", ((Long)mMap.get("CNT")).longValue());
            		map.put("message", (String)pMap.get("esmRuleGroupNm"));
            		map.put("ratio", (((Long)pMap.get("CNT")).longValue() * 100f) / ((Long)mMap.get("CNT")).longValue());
