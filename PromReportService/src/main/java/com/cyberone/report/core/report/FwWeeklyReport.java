...
public class FwWeeklyReport extends BaseReport {
	...
	public HashMap<String, Object> getDataSource(String sReportType, String sSearchDate, HashMap<String, Object> hMap, int ItemNo, List<String> contentsList) throws Exception {
		...
		contentsList.add(ItemNo + ". " + (String)hMap.get("assetName") + " 장비의 주간 세션로그 분석");
		reportData.put("RT", ItemNo + ". " + (String)hMap.get("assetName") + " 장비의 주간 세션로그 분석"); //Report Title
		...
		for (Entry<String, Object> e : hData.entrySet()) {
			...
			switch (e.getKey()) {
				...
				case "opt09" : 	//내부에서 외부로의 차단 세션 로그 발생추이
					...
					break;
				case "opt10" : 	//외부에서 내부로의 전체 세션로그 & SIP TOP
					...
					break;
				case "opt11" : 	//외부에서 내부로의 허용 세션로그 & SIP TOP
					...
					break;
				case "opt12" : 	//외부에서 내부로의 차단 세션로그 & SIP TOP
					...
					break;
				case "opt13" : 	//내부에서 외부로의 전체 세션로그 & SIP TOP
					...
					break;
				case "opt14" : 	//내부에서 외부로의 허용 세션로그 & SIP TOP
					...
					break;
				case "opt15" : 	//내부에서 외부로의 차단 세션로그 & SIP TOP
					...
					break;
				case "opt16" : 	//외부에서 내부로의 전체 세션로그 & DIP TOP
					...
					break;
				case "opt17" : 	//외부에서 내부로의 허용 세션로그 & DIP TOP
					...
					break;
				case "opt18" : 	//외부에서 내부로의 차단 세션로그 & DIP TOP
					...
					break;
				case "opt19" : 	//내부에서 외부로의 전체 세션로그 & DIP TOP
					...
					break;
				case "opt20" : 	//내부에서 외부로의 허용 세션로그 & DIP TOP
					...
					break;
				case "opt21" : 	//내부에서 외부로의 차단 세션로그 & DIP TOP
	...
	private void Action_SessionLog_Trend(HashMap<String, Object> reportData, int nDirection, String sAction, int assetCode, String sStartDay, String sEndDay) throws Exception {
		...
		Iterable<DBObject> dbResult = fwDao.CompareSessionLogTrend("DY", nDirection, sAction, assetCode, fwDao.addDate(sStartDay,-7), sEndDay);
	...
	private void Action_SessionLog_TopN(
    	...
    	Iterable<DBObject> prevObj = fwDao.BeforeIpCondition("DY", nDirection, sAction, assetCode, fwDao.addDate(sStartDay,-7), fwDao.addDate(sEndDay,-7), bSrcIp, topNIps);
	...
	private void Action_ServiceTop10(
    	...
    	Iterable<DBObject> prevResult = fwDao.ServiceSessionLog("DY", nDirection, sAction, assetCode, fwDao.addDate(sStartDay,-7), fwDao.addDate(sEndDay,-7));
