...
public class FwWeeklyReport extends BaseReport {
	...
	public HashMap<String, Object> getDataSource(String sReportType, String sSearchDate, HashMap<String, Object> hMap) throws Exception {
		logger.debug("방화벽 주간보고서 통계데이타 생성");
	...
	private void All_SessionLog_Trend(HashMap<String, Object> reportData, int nDirection, int assetCode, String sStartDay, String sEndDay, String sSearchPort) throws Exception {
		...
		//허용/차단
		Iterable<DBObject> dbResult = fwDao.SessionLogDayTrend(nDirection, ALLOW, assetCode, sStartDay, sEndDay, 0);
		...
		dbResult = fwDao.SessionLogDayTrend(nDirection, "", assetCode, sStartDay, sEndDay, 0);
    	...
    	if (searchPort != null) {
    		for (int i = 0; i < searchPort.length; i++) {
	    		...
	    		dbResult = fwDao.SessionLogDayTrend(nDirection, ALLOW, assetCode, sStartDay, sEndDay, searchPort[i]);
	...
	private void Action_SessionLog_Trend(HashMap<String, Object> reportData, int nDirection, String sAction, int assetCode, String sStartDay, String sEndDay) throws Exception {
		...
		Iterable<DBObject> dbResult = fwDao.CompareSessionLogDayTrend(nDirection, sAction, assetCode, fwDao.addDate(sStartDay,-7), sEndDay);
    	...
    	List<String> dayPeriod = fwDao.getPeriodDay(fwDao.addDate(sStartDay,-7), sEndDay, 1);
	...
	private void Action_SessionLog_TopN(
    	...
    	//IP 건수기준 TOP N 리스트
		Iterable<DBObject> ipTopNResult = fwDao.IpPeriodTopN(nDirection, sAction, assetCode, sStartDay, sEndDay, bSrcIp, nLimit);
    	...
    	for (DBObject val : ipTopNResult) {
    		...
    		if (bSrcIp) {    		
        		...
        		prevObj = fwDao.BeforeIpDayCondition(nDirection, sAction, assetCode, fwDao.addDate(sStartDay,-7), fwDao.addDate(sEndDay,-7), bSrcIp, (String)val.get("srcIp"));
    		} else {
    			topNIps.add((String)val.get("destIp"));
    			prevObj = fwDao.BeforeIpDayCondition(nDirection, sAction, assetCode, fwDao.addDate(sStartDay,-7), fwDao.addDate(sEndDay,-7), bSrcIp, (String)val.get("destIp"));
    		}
    		val.put("prevCount", 0);
	...
	private void Action_ServiceTop10(
    	...
    	Iterable<DBObject> prevResult = fwDao.ServiceSessionLogPeriod(nDirection, sAction, assetCode, fwDao.addDate(sStartDay,-7), fwDao.addDate(sEndDay,-7));
		for (DBObject obj : prevResult) {
			prevMap.put(String.valueOf((Integer)obj.get("destPort")), obj);
		}
    	
		Iterable<DBObject> logResult = fwDao.ServiceSessionLogPeriod(nDirection, sAction, assetCode, sStartDay, sEndDay);   	
