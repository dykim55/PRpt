...
public class IpsPeriodReport extends BaseReport {
	
	private IpsDao ipsDao;

	private UserInfo userInfo;
	
	public IpsPeriodReport(UserInfo userInfo) {
		this.userInfo = userInfo;
		this.ipsDao = new IpsDao(userInfo.getPromDb(), userInfo.getReportDb());
	}
	
	/* 
	 * 침입방지(IPS) 임의기간보고서 통계데이타 생성
	 */
	...
	public HashMap<String, Object> getDataSource(String sReportType, String sSearchDate, HashMap<String, Object> hMap, int ItemNo, List<String> contentsList) throws Exception {
		logger.debug("침입방지(IPS) 임의기간보고서 통계데이타 생성");
		
		HashMap<String, Object> reportData = new HashMap<String, Object>();
		
		reportData.put("reportType", sReportType);
		
		contentsList.add(ItemNo + ". " + (String)hMap.get("assetName") + " 장비의 IPS 탐지로그 분석");
		reportData.put("RT", ItemNo + ". " + (String)hMap.get("assetName") + " 장비의 IPS 탐지로그 분석");
		...
		for (Entry<String, Object> e : hData.entrySet()) {
			...
			switch (e.getKey()) {
				...
				case "opt04" : 	//외부에서 내부로의 전체 탐지로그 & 이벤트 TOP
					...
					break;
				case "opt05" : 	//외부에서 내부로의 허용 탐지로그 & 이벤트 TOP
					reportData.put("opt05", push(contentsList, "    ", ItemNo, nC2 + nC1, nS2++, " 외부에서 내부로의 허용 탐지로그에 대한 탐지룰 TOP 현황"));
					...
					break;
				case "opt06" : 	//외부에서 내부로의 차단 탐지로그 & 이벤트 TOP
					reportData.put("opt06", push(contentsList, "    ", ItemNo, nC2 + nC1, nS2++, " 외부에서 내부로의 차단 탐지로그에 대한 탐지룰 TOP 현황"));
					...
					break;
				case "opt07" : 	//내부에서 외부로의 전체 탐지로그 & 이벤트 TOP
					reportData.put("opt07", push(contentsList, "    ", ItemNo, nC2 + nC1, nS2++, " 내부에서 외부로의 전체 탐지로그에 대한 탐지룰 TOP 현황"));
					...
					break;
				case "opt08" : 	//내부에서 외부로의 허용 탐지로그 & 이벤트 TOP
					reportData.put("opt08", push(contentsList, "    ", ItemNo, nC2 + nC1, nS2++, " 내부에서 외부로의 허용 탐지로그에 대한 탐지룰 TOP 현황"));
					...
					break;
				case "opt09" : 	//내부에서 외부로의 차단 탐지로그 & 이벤트 TOP
					reportData.put("opt09", push(contentsList, "    ", ItemNo, nC2 + nC1, nS2++, " 내부에서 외부로의 차단 탐지로그에 대한 탐지룰 TOP 현황"));
					...
					break;
				case "opt10" : 	//외부에서 내부로의 전체 탐지로그 & SIP TOP
					reportData.put("opt10", push(contentsList, "    ", ItemNo, nC3 + nC2 + nC1, nS3++, " 외부에서 내부로의 전체 탐지로그에 대한 출발지IP TOP 현황"));
					nChoice = Integer.valueOf(StringUtil.convertString(hData.get("rd10")));
					Action_DetectLog_TopN(reportData, INBOUND, "", assetCode, sStartDay, sEndDay, true, nChoice, !StringUtil.isEmpty(hData.get("ck10")));
					break;
				case "opt11" : 	//외부에서 내부로의 허용 탐지로그 & SIP TOP
					reportData.put("opt11", push(contentsList, "    ", ItemNo, nC3 + nC2 + nC1, nS3++, " 외부에서 내부로의 허용 탐지로그에 대한 출발지IP TOP 현황"));
					nChoice = Integer.valueOf(StringUtil.convertString(hData.get("rd11")));
					Action_DetectLog_TopN(reportData, INBOUND, ALLOW, assetCode, sStartDay, sEndDay, true, nChoice, !StringUtil.isEmpty(hData.get("ck11")));
					break;
				case "opt12" : 	//외부에서 내부로의 차단 탐지로그 & SIP TOP
					reportData.put("opt12", push(contentsList, "    ", ItemNo, nC3 + nC2 + nC1, nS3++, " 외부에서 내부로의 차단 탐지로그에 대한 출발지IP TOP 현황"));
					nChoice = Integer.valueOf(StringUtil.convertString(hData.get("rd12")));
					Action_DetectLog_TopN(reportData, INBOUND, CUTOFF, assetCode, sStartDay, sEndDay, true, nChoice, !StringUtil.isEmpty(hData.get("ck12")));
					break;
				case "opt13" : 	//내부에서 외부로의 전체 탐지로그 & SIP TOP
					reportData.put("opt13", push(contentsList, "    ", ItemNo, nC3 + nC2 + nC1, nS3++, " 내부에서 외부로의 전체 탐지로그에 대한 출발지IP TOP 현황"));
					nChoice = Integer.valueOf(StringUtil.convertString(hData.get("rd13")));
					Action_DetectLog_TopN(reportData, OUTBOUND, "", assetCode, sStartDay, sEndDay, true, nChoice, !StringUtil.isEmpty(hData.get("ck13")));
					break;
				case "opt14" : 	//내부에서 외부로의 허용 탐지로그 & SIP TOP
					reportData.put("opt14", push(contentsList, "    ", ItemNo, nC3 + nC2 + nC1, nS3++, " 내부에서 외부로의 허용 탐지로그에 대한 출발지IP TOP 현황"));
					nChoice = Integer.valueOf(StringUtil.convertString(hData.get("rd14")));
					Action_DetectLog_TopN(reportData, OUTBOUND, ALLOW, assetCode, sStartDay, sEndDay, true, nChoice, !StringUtil.isEmpty(hData.get("ck14")));
					break;
				case "opt15" : 	//내부에서 외부로의 차단 탐지로그 & SIP TOP
					reportData.put("opt15", push(contentsList, "    ", ItemNo, nC3 + nC2 + nC1, nS3++, " 내부에서 외부로의 차단 탐지로그에 대한 출발지IP TOP 현황"));
					nChoice = Integer.valueOf(StringUtil.convertString(hData.get("rd15")));
					Action_DetectLog_TopN(reportData, OUTBOUND, CUTOFF, assetCode, sStartDay, sEndDay, true, nChoice, !StringUtil.isEmpty(hData.get("ck15")));
					break;
				case "opt16" : 	//외부에서 내부로의 전체 탐지로그 & DIP TOP
					reportData.put("opt16", push(contentsList, "    ", ItemNo, nC4 + nC3 + nC2 + nC1, nS4++, " 외부에서 내부로의 전체 탐지로그에 대한 목적지IP TOP 현황"));
					...
					break;
				case "opt17" : 	//외부에서 내부로의 허용 탐지로그 & DIP TOP
					reportData.put("opt17", push(contentsList, "    ", ItemNo, nC4 + nC3 + nC2 + nC1, nS4++, " 외부에서 내부로의 허용 탐지로그에 대한 목적지IP TOP 현황"));
					...
					break;
				case "opt18" : 	//외부에서 내부로의 차단 탐지로그 & DIP TOP
					reportData.put("opt18", push(contentsList, "    ", ItemNo, nC4 + nC3 + nC2 + nC1, nS4++, " 외부에서 내부로의 차단 탐지로그에 대한 목적지IP TOP 현황"));
					...
					break;
				case "opt19" : 	//내부에서 외부로의 전체 탐지로그 & DIP TOP
					reportData.put("opt19", push(contentsList, "    ", ItemNo, nC4 + nC3 + nC2 + nC1, nS4++, " 내부에서 외부로의 전체 탐지로그에 대한 목적지IP TOP 현황"));
					...
					break;
				case "opt20" : 	//내부에서 외부로의 허용 탐지로그 & DIP TOP
					reportData.put("opt20", push(contentsList, "    ", ItemNo, nC4 + nC3 + nC2 + nC1, nS4++, " 내부에서 외부로의 허용 탐지로그에 대한 목적지IP TOP 현황"));
					...
					break;
				case "opt21" : 	//내부에서 외부로의 차단 탐지로그 & DIP TOP
					reportData.put("opt21", push(contentsList, "    ", ItemNo, nC4 + nC3 + nC2 + nC1, nS4++, " 내부에서 외부로의 차단 탐지로그에 대한 목적지IP TOP 현황"));
					...
					break;
				case "opt22" : 	//외부에서 내부로의 전체 탐지로그 & 서비스 TOP10
					reportData.put("opt22", push(contentsList, "    ", ItemNo, nC5 + nC4 + nC3 + nC2 + nC1, nS5++, " 외부에서 내부로의 전체 탐지로그에 대한 서비스 TOP 현황"));
					ALL_ServiceTop10(reportData, INBOUND, assetCode, sStartDay, sEndDay, !StringUtil.isEmpty(hData.get("ck22")));
					break;
				case "opt23" : 	//내부에서 외부로의 전체 탐지로그 & 서비스 TOP10
					reportData.put("opt23", push(contentsList, "    ", ItemNo, nC5 + nC4 + nC3 + nC2 + nC1, nS5++, " 내부에서 외부로의 전체 탐지로그에 대한 서비스 TOP 현황"));
					ALL_ServiceTop10(reportData, OUTBOUND, assetCode, sStartDay, sEndDay, !StringUtil.isEmpty(hData.get("ck23")));
					break;
				case "opt99" :	//성능정보
	...
	private void All_DetectLog_Trend(HashMap<String, Object> reportData, int nDirection, int assetCode, String sStartDay, String sEndDay) throws Exception {
		...
		Iterable<DBObject> dbResult = ipsDao.AllDetectLogTrend("DY", -1, assetCode, sStartDay, sEndDay);
		...
		dbResult = ipsDao.AllDetectLogTrend("DY", 1, assetCode, sStartDay, sEndDay);
	...
	private void Direction_DetectLog_Trend(HashMap<String, Object> reportData, int nDirection, int assetCode, String sStartDay, String sEndDay) throws Exception {
		...
		Iterable<DBObject> dbResult = ipsDao.AllActionTrend("DY", nDirection, "", assetCode, sStartDay, sEndDay);
		...
		dbResult = ipsDao.AllActionTrend("DY", nDirection, ALLOW, assetCode, sStartDay, sEndDay);
    	...
    	if (bChk) { //Event Top 발생추이
	    	...
	    	if (nDirection == INBOUND) {
	    		if (sAction.equals(ALLOW)) {
			    	...
			    	reportData.put("OPT6_3", new SynthesisDataSource(dataSource3));
	    		}
	    	} else {
	    		if (sAction.equals(ALLOW)) {
	    			reportData.put("OPT8_0", "일");
			    	reportData.put("OPT8_3", new SynthesisDataSource(dataSource3));
	    		} else {
