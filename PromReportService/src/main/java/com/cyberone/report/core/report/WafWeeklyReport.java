...
public class WafWeeklyReport extends BaseReport {
	
	private WafDao wafDao;

	private UserInfo userInfo;
	
	public WafWeeklyReport(UserInfo userInfo) {
	...
	public HashMap<String, Object> getDataSource(String sReportType, String sSearchDate, HashMap<String, Object> hMap, int ItemNo, List<String> contentsList) throws Exception {
		logger.debug("웹방화벽 주간보고서 통계데이타 생성");
		
		HashMap<String, Object> reportData = new HashMap<String, Object>();
		
		reportData.put("reportType", sReportType);
		
		contentsList.add(ItemNo + ". " + (String)hMap.get("assetName") + " 장비의 주간 웹방화벽 탐지로그 분석");
		reportData.put("RT", ItemNo + ". " + (String)hMap.get("assetName") + " 장비의 주간 웹방화벽 탐지로그 분석"); //Report Title
		...
		for (Entry<String, Object> e : hData.entrySet()) {
			...
			switch (e.getKey()) {
				...
				case "opt01" : case "opt02" :
					if (nC1 == 0) { 
						reportData.put("C1", push(contentsList, "  ", ItemNo, ++nC1, " 주간 탐지 추이"));
					}
					break;
				//이벤트 현황
				case "opt03" :
					if (nC2 == 0) {
						reportData.put("C2", push(contentsList, "  ", ItemNo, (++nC2 + nC1), " 주간 탐지 현황"));
					}
					break;
				//출발지IP 현황
				case "opt04" :
					if (nC3 == 0) {
						reportData.put("C3", push(contentsList, "  ", ItemNo, (++nC3 + nC2 + nC1), " 주간 출발지 현황"));
					}
					break;
				//도메인별 상세통계
				case "opt05" : case "opt06" : case "opt07" :
					if (nC4 == 0) {
						reportData.put("C4", push(contentsList, "  ", ItemNo, (++nC4 + nC3 + nC2 + nC1), " 주간 목적지 현황"));
					}
					break;
				//성능정보
				case "opt99" :
	...
	private void Domain_EventTopN_Condition(HashMap<String, Object> reportData, int assetCode, String sStartDay, String sEndDay) throws Exception {
    	...
    	for (DBObject mVal : dbTopN) {
    		...
    		Iterable<DBObject> dbBefore = wafDao.DomainEventTopN("DY", assetCode, wafDao.addDate(sStartDay, -7), wafDao.addDate(sEndDay, -7), sHost, 0);
	...
	private void ALL_Event_TopN(HashMap<String, Object> reportData, int assetCode, String sStartDay, String sEndDay, int nLimit) throws Exception {
		...
		dbResult = wafDao.EventTopN("DY", assetCode, wafDao.addDate(sStartDay,-7), wafDao.addDate(sEndDay,-7), 0);
    	HashMap<String, DBObject> mapBefore = new HashMap<String, DBObject>(); 
    	for (DBObject val : dbResult) {
    		mapBefore.put((String)val.get("message"), val);
    	}
	...
	private void ALL_SrcIp_TopN(HashMap<String, Object> reportData, int assetCode, String sStartDay, String sEndDay, int nLimit, boolean bChk) throws Exception {
    	...
    	//전일 SrcIp TopN
    	Iterable<DBObject> dbBefore = wafDao.SrcIpTopN("DY", assetCode, wafDao.addDate(sStartDay, -7), wafDao.addDate(sEndDay, -7), 0, saTopN);
